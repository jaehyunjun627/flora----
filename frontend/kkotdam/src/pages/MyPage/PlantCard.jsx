...
const CARD_COLORS = [
  { name: '세이지 그린', bg: '#eef6ef', accent: '#4a7c59', text: '#2a2a2a' },
...
export default function PlantCard({ selectedBadge, plants = [], userName = '사용자' }) {
  ...
  const handleSaveImage = () => {
    ...
    const plantsHtml = plants.length > 0
      ? plants.map(p => `<span style="display:inline-block;background:${color.accent}22;color:${color.accent};border:1px solid ${color.accent}55;border-radius:20px;padding:3px 10px;font-size:12px;margin:2px;">🪴 ${p.nickname || p.name}</span>`).join(' ')
    ...
    win.document.write(`
      ...
      <style>
        body { margin: 0; display: flex; justify-content: center; align-items: center; min-height: 100vh; background: #f0f0f0; font-family: 'Apple SD Gothic Neo', sans-serif; }
  ...
  return (
    <div className="plant-card-section">
        ...
        <div className="pc-plants">
          {plants.length > 0 ? plants.map(p => (
            <span
              ...
              style={{ background: `${color.accent}22`, color: color.accent, borderColor: `${color.accent}55` }}
            >
              🪴 {p.nickname || p.name}
            </span>
          )) : (
      ...
      {!selectedBadge && (
        <p className="card-badge-hint">💡 위 뱃지 컬렉션에서 뱃지를 선택하면 명함에 표시돼요!</p>
      )}

      <div className="card-action-row">
        <button className="btn-card-action" onClick={handleSaveImage}>📥 이미지 저장</button>
        <button className="btn-card-action" onClick={handleShareLink}>🔗 링크 공유</button>
      </div>
    </div>
  );
}
