...
export default function MyCalendar({ plants, setPlants }) {
  ...
  const handleCheckIn = () => { setShowMission(true); };
      ...
      {selectedDay && (
        <div className="event-list">
          <div className="event-list-header">
            <h3 className="event-list-title">{currentMonth + 1}월 {selectedDay}일 일정</h3>
          ...
          {selectedEvents.length === 0 ? (
            ...
            selectedEvents.map(e => (
              <div key={e.id} className={`event-item${e.isCompleted ? ' completed' : ''}`}>
                <span className="event-color-dot" style={{ background: EVENT_TYPE_COLOR[e.type] || '#b8b8b8' }} />
        ...
        <div className="modal-overlay" onClick={() => setShowMission(false)}>
          <div className="modal mission-modal" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
              <button className="btn-primary" onClick={() => setShowMission(false)}>확인</button>
              {completedMissions.length > 0 && (
                <button className="btn-secondary" style={{ fontSize: 12, padding: '11px 12px' }} onClick={resetMissions}>
                  미션 초기화
                </button>
              )}
            </div>
      ...
      {showAddEvent && (
        <div className="modal-overlay" onClick={() => setShowAddEvent(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-actions">
              <button className="btn-primary" onClick={saveCustomEvent} disabled={!newEvent.title.trim()}>추가하기</button>
