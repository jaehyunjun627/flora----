...
function Layout({ children }) {
  ...
  const navLinks = [
    ...
    { path: '/products', label: '마켓', prefixMatch: true },  // category 파라미터 무관하게 /products면 항상 활성화
            ...
            <nav className="header-nav">
              {navLinks.map(link => (
                <Link
                  key={link.path}
                  to={link.path}
                  className={`header-nav-link ${(link.prefixMatch ? location.pathname === link.path : isActive(link.path)) ? 'active' : ''}`}
                >
                  {link.label}
                </Link>
              ))}
