...
export default function MyPage() {
  ...
  const [serverEvents,    setServerEvents]    = useState(null);   // 서버에서 받아온 events (최초 null = 미로드)
  ...
  // 서버에서 유저 캘린더(plants/events) 로드
  const loadUserCalendar = async () => {
    try {
      const res = await api.get('/api/user-calendar');
      const serverPlants = Array.isArray(res.data?.plants) ? res.data.plants : [];
      const serverEv     = Array.isArray(res.data?.events) ? res.data.events : [];
      // 서버에 저장된 식물이 있으면 서버를 신뢰, 없으면 로컬 초기값 유지(마이그레이션)
      if (serverPlants.length > 0) {
        setPlants(serverPlants);
      } else {
        // 로컬에 이미 있는 식물/일정을 최초 1회 서버로 업로드 (마이그레이션)
        try {
          const localEv = JSON.parse(localStorage.getItem('flora-events') || '[]');
          await api.put('/api/user-calendar', { plants, events: localEv });
        } catch (_) {}
      }
      setServerEvents(serverEv);
    } catch (e) {
      console.error('캘린더 로드 실패:', e);
      setServerEvents([]); // 실패해도 로컬로 동작하도록 빈 배열 전달
    }
  };

  useEffect(() => {
    if (authUser) loadUserCalendar();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [authUser]);

  // 출석 API 호출 → 포인트/뱃지/게이지 갱신
  const handleApiCheckIn = async () => {
    try {
      await api.post('/api/mypage/checkin');
      await loadMyPage(); // 최신 상태 재조회
    } catch (e) {
      console.error('출석 실패:', e);
    }
  };

  // 오늘 이미 API 출석했는지 확인
  const todayApiStr = new Date().toISOString().split('T')[0];
  const apiCheckedInToday = myData?.lastCheckIn === todayApiStr;
        ...
        {activeTab === 'activity' && (
          <div className="activity-tab">
            <MyCalendar
              plants={plants}
              setPlants={setPlants}
              onApiCheckIn={handleApiCheckIn}
              apiCheckedIn={apiCheckedInToday}
              serverEvents={serverEvents}
              onServerEventsChange={setServerEvents}
            />
            <div className="two-col-grid">
              <PointLevel points={userPoints} gauge={myData?.gauge} />
