...
export default function SubscriptionCheckoutPage() {
  ...
  const processPayment = async () => {
    ...
    try {
      ...
      // 단일 구매(예약 발송) 또는 구독 첫 회차 — 백엔드에 주문 등록해서
      // 주문 내역(/orders) 에서 조회되게 한다.
      try {
        const fullAddress = [deliveryInfo.address, deliveryInfo.addressDetail]
          .filter(Boolean).join(' ').trim();
        const memo = deliveryInfo.memo === '직접 입력'
          ? deliveryInfo.memoCustom
          : deliveryInfo.memo;
        const productLabel = isSingle
          ? `${plan.name} (예약 발송)`
          : `${plan.name} (구독 ${plan.periodLabel})`;
        const orderRes = await api.post('/api/orders/scheduled', {
          ...
          deliveryMemo: memo,
          paymentMethod,
          // 단일 구매면 사용자가 고른 deliveryDate, 구독이면 첫 회차로 오늘+3일
          scheduledDeliveryDate: isSingle
            ? deliveryDate
            : (() => {
        ...
        backendOrderNumber = orderRes.data?.orderNumber;
      } catch (apiErr) {
        console.warn('주문 백엔드 등록 실패 (로컬 처리 계속):', apiErr);
      }

      const newOrderId = backendOrderNumber || ('SUB-' + Date.now());
