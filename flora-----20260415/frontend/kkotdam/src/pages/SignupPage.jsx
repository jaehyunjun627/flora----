...
// 회원가입 시 한 번에 모든 약관 동의를 받음 (마이페이지에서는 확인만)
const TERMS_LIST = [
  { key: 'terms',     label: '이용약관 동의',              required: true },
  { key: 'privacy',   label: '개인정보 처리방침 동의',     required: true },
  { key: 'ai',        label: 'AI 진단 서비스 이용약관',    required: true },
  { key: 'pet',       label: '반려동물 안전 정보 이용약관', required: true },
  { key: 'location',  label: '위치정보 이용 동의',         required: false },
...
function SignupPage() {
  ...
  const [agrees, setAgrees] = useState(() =>
    TERMS_LIST.reduce((acc, t) => ({ ...acc, [t.key]: false }), {})
  );
  ...
  const handleAllAgree = () => {
    const allChecked = TERMS_LIST.every(t => agrees[t.key]);
    const next = TERMS_LIST.reduce((acc, t) => ({ ...acc, [t.key]: !allChecked }), {});
    setAgrees(next);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const missingRequired = TERMS_LIST.filter(t => t.required).some(t => !agrees[t.key]);
    if (missingRequired) {
    ...
    try {
      await signup(
        ...
        form.role, form.businessName, form.businessNumber, agrees
      );
      navigate('/');
    } catch (err) {
  ...
  const allAgreed = TERMS_LIST.every(t => agrees[t.key]);
    ...
    <div className="signup-container">
      <div className="signup-card">
        ...
        <form onSubmit={handleSubmit} className="signup-form">
          ...
          {/* 약관 동의 (필수/선택 모두 회원가입 시 일괄 수집) */}
          <div className="agree-box">
            <label className="agree-all-label" onClick={handleAllAgree}>
              <span className={`agree-checkbox ${allAgreed ? 'checked' : ''}`}>
                {allAgreed ? '✓' : ''}
              </span>
              <span className="agree-all-text">전체 동의 (선택 약관 포함)</span>
            </label>
            <div className="agree-divider" />
            {TERMS_LIST.map(t => (
              <label
                key={t.key}
                className="agree-item-label"
                onClick={() => handleAgreeChange(t.key)}
              >
                <span className={`agree-checkbox ${agrees[t.key] ? 'checked' : ''}`}>
                  {agrees[t.key] ? '✓' : ''}
                </span>
                <span className="agree-item-text">
                  <span className={t.required ? 'agree-required' : 'agree-optional'}>
                    [{t.required ? '필수' : '선택'}]
                  </span> {t.label}
                </span>
