...
function generateScheduleFromAi(plant, aiResult) {
  ...
  for (let i = 1; i <= 4; i++) {
    ...
    date.setDate(today.getDate() + (aiResult.wateringInterval || 7) * i);
    schedules.push({
      id: Date.now() + i,
      plantId: plant.id,
  ...
  repotDate.setDate(today.getDate() + (aiResult.repottingInterval || 180));
  schedules.push({
    id: Date.now() + 100,
    plantId: plant.id,
    plantName: plant.name,
...
/**
 * 일정 메모 블록 - 날짜 무관하게 언제든 메모 작성/수정 가능
 * 접혀있다가 "📝 메모" 버튼 클릭 시 펼쳐짐
 */
function EventMemo({ event, onSave }) {
  const [open, setOpen]   = useState(!!event.memo);
  const [value, setValue] = useState(event.memo || '');
  ...
  return (
    <div className="event-memo-wrap" style={{ width: '100%', marginTop: 6 }}>
      <button
        className="event-memo-toggle"
        onClick={(ev) => { ev.stopPropagation(); setOpen(o => !o); }}
        style={{
          fontSize: 12,
          background: event.memo ? '#fff5d6' : '#f5f2ed',
          color: '#7a6a50',
      ...
      {open && (
        <div style={{ marginTop: 6, display: 'flex', flexDirection: 'column', gap: 4 }}>
          <textarea
            ...
            placeholder="예) 오늘 하루 일찍 물을 줬어요 / 오후에 잎이 축 처져 있어 물을 조금 더 줌"
            rows={2}
            style={{
              width: '100%',
              border: '1px solid #e0d8c8',
          ...
          <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
            {saved && <span style={{ fontSize: 11, color: '#4a7c59', alignSelf: 'center' }}>✓ 저장됨</span>}
            <button
              className="btn-memo-save"
              onClick={(ev) => { ev.stopPropagation(); handleSave(); }}
              style={{
                fontSize: 12,
                background: '#4a7c59',
                color: '#fff',
...
function getInitialEvents() {
  ...
  return [
    {
      id: 1, plantId: 1, plantName: '내 식물', type: 'WATERING',
      title: '💧 첫 번째 식물 물주기',
...
export default function MyCalendar({ plants, setPlants, onApiCheckIn, apiCheckedIn, serverEvents, onServerEventsChange }) {
  ...
  // events: 서버에서 받아온 값(serverEvents)이 있으면 그걸 사용, 없으면 로컬 캐시 또는 초기값
  ...
  // 서버 events가 내려오면 그 값으로 대체 (최초 1회)
  useEffect(() => {
    if (Array.isArray(serverEvents)) {
      setEvents(serverEvents.length > 0 ? serverEvents : events);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [serverEvents]);

  // 캐시용 로컬 저장 유지 (오프라인/재방문 즉시 표시)
  useEffect(() => {
    localStorage.setItem('flora-events', JSON.stringify(events));
  }, [events]);

  // 서버에 events 전체 upsert (debounced via setTimeout microtask)
  const persistEvents = async (nextEvents) => {
    try {
      await api.put('/api/user-calendar', { plants, events: nextEvents });
      if (onServerEventsChange) onServerEventsChange(nextEvents);
    } catch (e) {
      console.error('일정 저장 실패(로컬만 유지):', e);
    }
  };
  // 서버에 plants + events 전체 upsert
  const persistAll = async (nextPlants, nextEvents) => {
    try {
      await api.put('/api/user-calendar', { plants: nextPlants, events: nextEvents });
      if (onServerEventsChange) onServerEventsChange(nextEvents);
    } catch (e) {
      console.error('캘린더 저장 실패(로컬만 유지):', e);
    }
  };
  ...
  const [deletePlantTarget, setDeletePlantTarget] = useState(null);
  ...
  const isQuizDone = () => {
    if (localStorage.getItem(`flora-quiz-done-${todayStr}`)) return true;
    try {
      const saved = JSON.parse(localStorage.getItem(`flora-quiz-${todayStr}`));
      return !!saved?.answered;
    } catch { return false; }
  };
  const [quizDoneToday, setQuizDoneToday] = useState(() => isQuizDone());

  // 탭 포커스 시 퀴즈 완료 재확인
  useEffect(() => {
    const check = () => setQuizDoneToday(isQuizDone());
  ...
  const toggleComplete = (eventId) => {
    setEvents(prev => {
      ...
      // AI 생성 일정은 당일(date === todayStr)에만 완료/해제 가능
      if (target.isAiGenerated && target.date !== todayStr) {
        alert(
          target.date < todayStr
            ? 'AI 일정은 지난 날짜에 체크할 수 없어요. 당일에만 완료 체크가 가능합니다.'
            : 'AI 일정은 미리 체크할 수 없어요. 당일이 되면 완료 체크할 수 있어요.'
        );
        return prev;
      }
  ...
  // 출석하기 - 로컬 캘린더 표시 + API 연동
  const hasTodayAttendance = attendance.includes(todayStr) || apiCheckedIn;
  ...
  const handleCheckIn = async () => {
    if (!hasTodayAttendance) {
      ...
      // API 체크인 호출 (부모에서 전달된 함수)
      if (onApiCheckIn) await onApiCheckIn();
    }
    setShowMission(true);
  };
  ...
  const resetMissions = () => {
    ...
    if (attendance.includes(todayStr) && !apiCheckedIn) {
      const updatedAttendance = attendance.filter(d => d !== todayStr);
      setAttendance(updatedAttendance);
      localStorage.setItem('flora-attendance', JSON.stringify(updatedAttendance));
    }
  ...
  const getAttendanceState = (day) => {
    const dateStr = `${currentYear}-${String(currentMonth + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
    if (attendance.includes(dateStr) || (dateStr === todayStr && apiCheckedIn)) return 'attended';
  ...
  const handleAddPlant = async () => {
    ...
    // 1) 종 기본값 산출 (AI 또는 fallback 테이블)
    let aiResult = null;
    try {
      const res = await api.post('/api/plants/analyze', {
      ...
      aiResult = AI_SCHEDULE_BY_TYPE[newPlant.plantType] || AI_SCHEDULE_BY_TYPE['기타'];
    }

    // 종 기본 cycle 으로 바로 일정 생성
    const plant = { ...newPlant, id: Date.now() };
    const newEvents = generateScheduleFromAi(plant, aiResult);
    const nextPlants = [...plants, plant];
    const nextEvents = [...events, ...newEvents];
    setPlants(nextPlants);
    setEvents(nextEvents);
    persistAll(nextPlants, nextEvents);
    // 모달 종료 + 상태 초기화
  ...
  const confirmDeletePlant = () => {
    ...
    const nextPlants = plants.filter(p => p.id !== deletePlantTarget.id);
    const nextEvents = events.filter(e => e.plantId !== deletePlantTarget.id);
    setPlants(nextPlants);
    setEvents(nextEvents);
    persistAll(nextPlants, nextEvents);
  ...
  return (
    <div className="my-calendar compact">
      ...
      <div className="plant-list">
        {plants.length === 0 && (
          <span className="plant-chip-empty">식물을 추가해보세요 🌱</span>
        )}
        {plants.map(p => (
          <button
            key={p.id}
        ...
        <div className="event-list">
          ...
          {selectedEvents.length === 0 ? (
            ...
            selectedEvents.map(e => {
              const aiLocked = e.isAiGenerated && e.date !== todayStr;
              return (
              <div key={e.id} className={`event-item${e.isCompleted ? ' completed' : ''}`}>
                ...
                {aiLocked && (
                  <span
                    ...
                    title={e.date < todayStr ? '지난 AI 일정은 체크할 수 없어요' : '당일에만 체크할 수 있어요'}
                  >
                    🔒 당일만
                  </span>
                )}
                <button
                  ...
                  disabled={aiLocked}
                  onClick={ev => { ev.stopPropagation(); if (!aiLocked) toggleComplete(e.id); }}
                  style={aiLocked ? { opacity: 0.4, cursor: 'not-allowed' } : undefined}
                  title={aiLocked ? '당일에만 완료 체크가 가능해요' : ''}
                >
                  {e.isCompleted ? '✅' : '○'}
                </button>
                <button
                  ...
                  onClick={ev => {
                    ev.stopPropagation();
                    setEvents(prev => {
                      const next = prev.filter(ev2 => ev2.id !== e.id);
                      persistEvents(next);
                      return next;
                    });
                  }}
                ...
                {/* 메모 기능 - 날짜 상관없이 언제든 작성 가능 */}
                <EventMemo
                  event={e}
                  onSave={(memo) => {
                    setEvents(prev => {
                      const next = prev.map(x => x.id === e.id ? { ...x, memo } : x);
                      persistEvents(next);
                      return next;
                    });
                  }}
