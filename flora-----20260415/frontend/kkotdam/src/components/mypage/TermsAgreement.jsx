...
export default function TermsAgreement() {
  // 서버의 agreedTerms (회원가입 시 동의한 목록)를 기준으로 초기화 — 읽기 전용
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const csv = res.data?.agreedTerms || '';
        const serverMap = {};
        csv.split(',').map(s => s.trim()).filter(Boolean).forEach(k => {
          // SignupPage의 key 체계: terms/privacy/ai/pet/location/marketing
          // TermsAgreement id 체계: service/privacy/location/ai/pet/marketing
          const id = k === 'terms' ? 'service' : k;
          serverMap[id] = true;
        });
        setAgreements(serverMap);
        if (res.data?.createdAt) {
          setAgreedAt(new Date(res.data.createdAt).toLocaleDateString('ko-KR'));
        }
      } catch (e) {
        // 비회원/네트워크 오류 시 빈 객체
        setAgreements({});
  ...
  // 약관 동의는 회원가입 시점에만 가능하고, 이후 철회/해제할 수 없음 (읽기 전용)
  const requiredTerms = TERMS_DATA.filter(t => t.required);
  const allRequiredAgreed = requiredTerms.every(t => agreements[t.id]);

  return (
    <div className="terms-section">
      <div className="terms-info-banner" style={{
        background: '#eef6ef', border: '1px solid #cfe4cf', borderRadius: 8,
        padding: '10px 14px', marginBottom: 12, fontSize: 13, color: '#4a7c59',
      }}>
        <span role="img" aria-label="lock" style={{ marginRight: 8 }}>🔒</span>
        회원가입 시 동의한 약관 내역입니다{agreedAt ? ` (동의일: ${agreedAt})` : ''}. 동의한 약관은 철회할 수 없으며, 탈퇴를 통해서만 철회됩니다.
      </div>
      ...
      <div className="terms-list">
        {TERMS_DATA.map(term => {
          ...
          return (
            <div key={term.id} className={`terms-card${agreed ? ' agreed' : ''}`} style={{ opacity: agreed ? 1 : 0.7 }}>
              <div
              ...
              >
                <button
                  className={`terms-check-btn${agreed ? ' checked' : ''}`}
                  disabled
                  style={{ cursor: 'not-allowed', opacity: 1 }}
                  title="동의한 약관은 철회할 수 없습니다"
                >
                  {agreed ? '✓' : '✕'}
                </button>
